...
import { LayoutDashboard, LogIn, Lock, Mail } from 'lucide-react'
import API from '../api'
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center bg-slate-50 dark:bg-slate-950 overflow-hidden font-sans">
      ...
      <div className="relative z-10 w-full max-w-md p-8 md:p-10 bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-gray-100 dark:border-slate-800">
        ...
        <p className="text-center text-gray-600 dark:text-slate-400 mb-8 font-medium">Log in to manage your team's tasks.</p>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-slate-950 border border-gray-100 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all font-medium text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-slate-500"
              value={email}
          ...
          <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-900/50 transition-all flex items-center justify-center gap-2">
            Sign In <LogIn className="w-5 h-5" />
          </button>
        </form>

        <p className="text-center mt-8 text-gray-600 dark:text-slate-400 font-medium">
          Don't have an account? <Link to="/signup" className="text-blue-400 hover:text-blue-300 transition-colors font-bold">Sign up here</Link>
        </p>
      </div>
    </div>
  )
